...
function createSongItem(songText,icon,capsule, ?vip, ?updated, ?isNew) {
	capsuleImage= Assets.exists(Paths.image("menus/freeplay/silhouettes/" + capsule))? capsule:'placeholder';
	var capsuleSprite:FlxSprite = new FlxSprite().loadGraphic(Paths.image("menus/freeplay/silhouettes/"+capsuleImage));
	capsuleSprite.scale.set(0.6, 0.6); capsuleSprite.updateHitbox();
