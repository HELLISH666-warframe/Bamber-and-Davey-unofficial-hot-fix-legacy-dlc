...
var curWeek=FlxG.save.data.Bamber_FREEPLAY_WEEK;
...
function create() {
	...
	text.text = FlxG.save.data.Bamber_FREEPLAY_WEEK.name;
	text.camera = coolCam;
	text.alignment="center";
	add(text);
	text.screenCenter(FlxAxes.X);

	for (e in curWeek.difficulties) {
	...
	trace("\n"+FlxG.save.data.Bamber_FREEPLAY_WEEK.name+"\n"
	+FlxG.save.data.Bamber_FREEPLAY_WEEK.difficulties);
}
...
function changeDiff(e) {
	...
	curDifficulty += e;
	if (curDifficulty < 0) curDifficulty = FlxG.save.data.Bamber_FREEPLAY_WEEK_DIFF.length-1;
	if (curDifficulty >= FlxG.save.data.Bamber_FREEPLAY_WEEK_DIFF.length) curDifficulty = 0;
	if (__oldDiffName != (__oldDiffName = FlxG.save.data.Bamber_FREEPLAY_WEEK_DIFF[curDifficulty].toLowerCase())) {
...
function changeOption(p) {
	...
	trace(curWeek.difficulties[curDifficulty]);
}
function toggle() {
	switch(curOption){
...
function playWeek() {
	PlayState.loadWeek(curWeek,curWeek.difficulties[curDifficulty].toLowerCase());
