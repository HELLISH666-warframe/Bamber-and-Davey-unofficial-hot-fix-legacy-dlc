...
var composer_icons = new FlxTypedGroup();
...
function create() {
	...
	add(composer_icons);
	for (z in curSong.freeplayShit.composer.split(",")){
		for (i in 0...z.length) {
		var songText = new FlxSprite(530,0);
		if (!Assets.exists(doesIconExist(z)))
			songText.loadGraphic(Paths.image('credits/missing'));
		else
			songText.loadGraphic(doesIconExist(z));
		if(z.length>2)songText.scale.set(0.4,0.4);
		composer_icons.add(songText).camera = coolCam;
	}
	}
	add(composer_icon);
	...
	var testtt=new ComposerIcon(830,200,'Glitch.smh,Walto');
	add(testtt.siloSprite).camera=coolCam;
}
