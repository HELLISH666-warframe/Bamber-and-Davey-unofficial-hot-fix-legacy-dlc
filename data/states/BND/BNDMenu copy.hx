...
function create() {
    ...
    test_thing=[buttonGroup,menuSelection];
}
...
function setupMenuStuff() {
    ...
    test_thing=[bottomMenuGroup,menuSelection];
}
...
function setupSubMenuStuff() {
    ...
    test_thing=[buttonSubgroup,menuSubmenuSelection];
}
...
function changeSelection(change = 0) {
    var oldMenuSelection = test_thing[1];
    //var oldSubMenuSelection = test_thing[1];
    test_thing[1] = FlxMath.wrap(test_thing[1]+change, 0, menuOptions.length - 1);
	trace("selecting");
    for (i in test_thing[0].members) {
        if (test_thing[1] == i.ID) 
		{
        ...
        i.animateAtlas.anim.play("Button", true, test_thing[1] == i.ID ? false : true, test_thing[1] == i.ID ? i.animateAtlas.anim.curFrame - i.animateAtlas.anim.length : i.animateAtlas.anim.curFrame + i.animateAtlas.anim.length );
    }

    buttonTextGroup.members[oldMenuSelection].alpha = 0;
    buttonTextGroup.members[test_thing[1]].alpha = 1;
...
function progressForwards() {
    if (!isInMenu) {
		...
		new FlxTimer().start(1, ()->{
		switch (test_thing[1]) {
		case 0: import funkin.menus.StoryMenuState;
...
var test_thing=[buttonGroup,menuSelection];
...
function postUpdate(elapsed) {
    if (initialized) {
        ...
        test_thing[0].forEach(function (button) {
            ...
            button.x = CoolUtil.fpsLerp(button.x, (button.ID == 0 ? 20 : (test_thing[0].members[button.ID - 1].x + test_thing[0].members[button.ID - 1].width) + 10), 0.33);
            button.y = bottomBar.y + 20 - button.height;

            if (isInMenu && FlxG.mouse.visible && FlxG.mouse.overlaps(button) && occupiedObject == null) {
				hoveringOverButton = true;
				trace("hovering");
                if (test_thing[1] != button.ID) {
                    FlxG.sound.play(Paths.sound('firstTime/firstButtonScroll'), getVolume(0.8, 'sfx'));
                    changeSelection(button.ID - test_thing[1]);
                }
