...
public function changeStrumLineColors(strumLine, ?char) {
    ...
    for (strumLine in strumLines)
       for (note in strumLine.notes) {
        note.shader= new CustomShader("ColoredNoteShader");
        noteShaders[i][0].r = ((colorArray[0] >> 16) & 0xFF);
        noteShaders[i][0].g = ((colorArray[0] >> 8) & 0xFF);
        noteShaders[i][0].b = ((colorArray[0]) & 0xFF);
    }
}
