...
circler1.camera = camHUD;circler2.cameras = [camHUD];circler3.cameras = [camHUD];
  ...
  //  if (FlxG.save.data.vs_bamber_modChart[PlayState.SONG.song]) {
        if (enableAtom) {
            for (circle in 0...circlerObjects.length) {
                circlerObjects[circle].y = allStrumsY[0] + Math.sin((Conductor.songPosition - startTimer) / 250 + (Math.PI / 3 * circle * mod_spacing)) * (mod_yAmount + mod_xAmount * 2) * (PlayState.downscroll ? 1 : -1) + mod_xAmount / 8;
                ...
                circlerObjects[circle].x += allStrumsY[0] + Math.cos((Conductor.songPosition - startTimer) / 500 + (Math.PI / 3 * circle * mod_spacing)) * (mod_yAmount) * (PlayState.downscroll ? 1 : -1) - (1280 / 4);
                circlerObjects[circle].y += allStrumsX[0] + Math.cos((Conductor.songPosition - startTimer) / 500 + (Math.PI / 3 * circle * mod_spacing)) * (mod_yAmount) * mod_speed - 720 / 2 * 1.65;
            }
        }

        for (strum in allStrums) {
            if (enableCircle) {
                strum.y = defaultStrumY + (mod_yAmount * (PlayState.downscroll ? -1 : 1) + Math.sin((Conductor.songPosition - startTimer) / 500 + (Math.PI / 8 * (i + (i > 3 ? 4 : 0)) * mod_spacing)) * mod_yAmount * (PlayState.downscroll ? 1 : -1));
                strum.x = (1280 / 2 - strum.width / 2) + Math.cos((Conductor.songPosition - startTimer) / 500 + (Math.PI / 8 * (i + (i > 3 ? 4 : 0)) * mod_spacing)) * -mod_xAmount * mod_speed;
                strum.scale.x = 0.8 + Math.sin((Conductor.songPosition - startTimer) / 500 + (Math.PI / 8 * (i + (i > 3 ? 4 : 0)) * mod_spacing)) * 0.2;
                strum.scale.y = 0.8 + Math.sin((Conductor.songPosition - startTimer) / 500 + (Math.PI / 8 * (i + (i > 3 ? 4 : 0)) * mod_spacing)) * 0.2;

                if (enableSpinAddon) {
                    strum.x += (mod_yAmount + Math.sin((Conductor.songPosition - startTimer) / 500 / 3 * 4 + (Math.PI * 2 / 8 * i * mod_spacing)) * mod_yAmount/2) - (1280/ 4);
                    strum.y += (1280 / 2 - strum.width / 2) + Math.sin((Conductor.songPosition - startTimer) / 500 / 3 * 4 + (Math.PI * 2 / 8 * i * mod_spacing)) * -mod_xAmount/2 * mod_speed * (PlayState.downscroll ? 1 : -1) - 720 / 2 * 1.58;
                }
            }

            if (enableYWave) {
                allStrumsY[i] = defaultStrumY + (Math.sin((Conductor.songPosition - startTimer) / 500 + (Math.PI * 2 / 8 * i * mod_spacing))) * mod_yAmount * (PlayState.downscroll ? -1 : 1);
            }
            ...
            if (enableXScroll) {
                ...
                else if (mod_speed >= 0 && allStrumsX[i] >= 1280) {
                    allStrumsX[i] = 0 - strum.width;
                    strum.x = allStrumsX[i];
                }

                allStrumsX[i] += mod_xAmount * mod_speed;
                //allStrumsX[i] += 9 * mod_speed;
                strum.angle = mod_xAmount * mod_speed * 4 * (PlayState.downscroll ? -1 : 1);
                //strum.angle = 9 * mod_speed * 4 * (PlayState.downscroll ? -1 : 1);
                strum.x = FlxMath.lerp(strum.x, allStrumsX[i], 0.4);
            }

            if (enableStillStrum) {
                allStrumsY[i] += Math.pow(FlxMath.roundDecimal(strum.getScrollSpeed(), 2), 2) * (PlayState.downscroll ? -2 : 2);
            ...
            if (enableAtom) {
                if (strum.ID == 0) {
                    ...
                    //strum.noteScale = 0.8 + Math.sin((Conductor.songPosition - startTimer) / 1000 + (Math.PI * 2 / (8 / 4) * (i % 3) * mod_spacing)) * 0.2;
                } else {
                   ...
                   // strum.noteScale = 0.6 + Math.sin((Conductor.songPosition - startTimer) / 750 + (Math.PI * 2 / (8 / 4) * ((i - strum.ID) % 3) * mod_spacing)) * 0.2;
                }
            }
          ...
          //  strum.noteScale = FlxMath.lerp(strum.notesScale, 1, 0.1);
...
function stepHit(curStep) {
    //if (FlxG.save.data.vs_bamber_modChart[PlayState.SONG.song.toLowerCase()]) {
        if (enableStillStrum && (curStep % 16 == 0 || curStep % 16 == 6 || curStep % 16 == 10)) {
            var i = 0;

            for (strum in allStrums) {
                allStrumsY[i] = defaultStrumY + (curStep % 16 == 0 ? 60 : curStep % 16 == 6 ? 100 : 200) * (PlayState.downscroll ? 1 : -1);
                strum.y = allStrumsY[i];
                i++;
            }
        }
        ...
        if (enableKickBop && (curStep % 16 == 0 || curStep % 16 == 6 || curStep % 16 == 10)) {
            ...
            for (strum in allStrums) {
                if (curStep % 16 == 0 || curStep % 16 == 10) {
                    strum.y = defaultStrumY + 60 * ((strum.ID == 1 || strum.ID == 2) ? 1 : -0.3) * (PlayState.downscroll ? 1 : -1);
                    ...
                    strum.y = defaultStrumY - 18 * ((strum.ID == 1 || strum.ID == 2) ? 1 : 0) * (PlayState.downscroll ? 1 : -1);
...
function prepareForSpin(loop) {
    ...
    switch (loop) {
        case '1':
            for (strum in allStrums) {
                strum.y = allStrumsY[i] - (120 / 4 * (4 - strum.ID)) * (PlayState.downscroll ? 1 : -1);
                strum.scale.set(0.95 - (0.05 * (4 - strum.ID)), 1.05 + (0.05 * (4 - strum.ID)));
                i++;
            }
        case '2':
            for (strum in allStrums) {
                strum.y = allStrumsY[i] + (30 * strum.ID) * (PlayState.downscroll ? 1 : -1);
        ...
        case '3':
            for (strum in allStrums) {
                ...
                strum.x = 1280 - 30 - (10 * (8 - i));
                allStrumsX[i] = 1280 - 100 - (100 * (8 - i));
                i++;
            }
        case '4':
            for (strum in allStrums) {
                ...
                FlxTween.tween(strum, {x: 1280 / 2 - (20 * (8 - i)), angle: -35}, Conductor.stepCrochet / 500, {ease: FlxEase.circIn, startDelay: Conductor.stepCrochet / 500});
...
function clump() {
    /*if (save.data.vs_bamber_modChart[PlayState.SONG.song.toLowerCase()]) {*//*
        var i = 0;
        for (strum in allStrums) {
            allStrumsX[i] = 1280 / 2 - strum.width / 2;
            allStrumsY[i] = 720 / 2 - strum.height / 2;
...
function switchModchartMovement(selectedMovements:String, allowBoom:String, disableEverything:String) {
    ...
    for (option in 0...movementArray.length) {
        ...
        switch (curOption) {
            ...
            case 'spin3d':
                for (strum in allStrums) {
                    /*if (save.data.vs_bamber_modChart[PlayState.SONG.song.toLowerCase()]) *//*strum.x = 1280 / 2 - strum.width / 2;
                    allStrumsX[index] = strum.x;
                    index++;
                }
                enableCircle = true;
                executeEvent({name: "Scroll Speed Change", params: [false,4,4,"quartInOut"
                ]});
            ...
            case 'XScroll':
                for (strum in allStrums) {
                    allStrumsX[index] = (1280 + 60) / 8 * index;
                    index++;
                }
                enableXScroll = true;
            case 'kickBop':
                ...
                executeEvent({name: "Scroll Speed Change", params: [false,3,4,"quartInOut"]});
            case 'stillStrum':
                enableStillStrum = true;
            case 'star':
                enableStar = true;
                for (strum in allStrums) {
                    allStrumsX[index] = 1280 / 2 - strum.width / 2 - (strum.ID == 0 || strum.ID == 3 ? (strum.width * 0.6) * (strum.ID == 0 ? 1 : -1) : 0);
                    allStrumsY[index] = 720 / 2 - strum.height / 2 - (strum.ID == 1 || strum.ID == 2 ? (strum.height * 0.6) * (strum.ID == 2 ? 1 : -1) : 0);

                    var fuck:Bool=true;
                   strum.noteAngle = fuck ? (switch (strum.ID) {
                        case 0: 270;
                        case 1: 180;
                        case 2: 0;
                        case 3: 90;
                    } + (PlayState.downscroll ? 0 : 180)) : 0;
